...
export function buildLiveReport({ spineRows, dailyRowsByDay, costCenters, orgMembersByOrg = {}, seats: rawSeats = {}, manifest }) {
  ...
  const focusRows = facts.map((row) => ({
    ...
    x_github_ai_credits: row.ai_credits_used,
    x_github_cost_center: row.cost_center_name,
    x_github_model: "",
    x_github_harness: row.used_cli ? "copilot_cli" : "",
    x_github_user: row.user_login,
    x_github_day: row.day,
  }));
